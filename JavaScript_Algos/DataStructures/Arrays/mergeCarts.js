...
//! DO NOT compare products by reference, assume each list came from an API / DB so they are fresh copies of the products and not references.
  
// Creates a new array of products containing all the shoppingCart items and any wishList item that wasn't already in the shoppingCart.
  function mergeCarts2 (shoppingCart, wishList){
    for ( let i = 0; i < wishList.length; i++){
        if (!shoppingCart.includes(wishList[i])) {
            shoppingCart.push(wishList[i]);
        }
    }
    return shoppingCart;
  }

  function mergeCarts3(shoppingCart, wishList) {
    let result = [];
    shoppingCart.map(item => result.push(item));
    wishList.filter(item => !shoppingCart.includes(item)).map(item => result.push(item));
    return result;
  }

  function mergeCarts(shoppingCart, wishList) {

    wishList.filter(item => !shoppingCart.includes(item)).map(item => shoppingCart.push(item));
  ...
  console.log(2, mergeCarts2(shoppingCart1, wishList1));
  console.log(3, mergeCarts3(shoppingCart1, wishList1));
