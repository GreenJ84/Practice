...
var _GradeSchool_sRoster;
const deepClone = (obj) => JSON.parse(JSON.stringify(obj));
export class GradeSchool {
    constructor() {
        _GradeSchool_sRoster.set(this, void 0);
        __classPrivateFieldSet(this, _GradeSchool_sRoster, {}, "f");
    }
    roster() {
        return deepClone(__classPrivateFieldGet(this, _GradeSchool_sRoster, "f"));
    }
    add(name, grade) {
        Object.keys(__classPrivateFieldGet(this, _GradeSchool_sRoster, "f")).forEach((value) => {
            __classPrivateFieldGet(this, _GradeSchool_sRoster, "f")[parseInt(value)] = __classPrivateFieldGet(this, _GradeSchool_sRoster, "f")[parseInt(value)].filter((alumn) => alumn !== name);
        });
        if (__classPrivateFieldGet(this, _GradeSchool_sRoster, "f")[grade]) {
            __classPrivateFieldGet(this, _GradeSchool_sRoster, "f")[grade].push(name);
            __classPrivateFieldGet(this, _GradeSchool_sRoster, "f")[grade].sort();
        }
        else {
            __classPrivateFieldGet(this, _GradeSchool_sRoster, "f")[grade] = [name];
        }
    }
    grade(grade) {
        if (__classPrivateFieldGet(this, _GradeSchool_sRoster, "f")[grade]) {
            let shell = __classPrivateFieldGet(this, _GradeSchool_sRoster, "f")[grade];
            shell.sort();
            return [...shell];
        }
        else {
...
_GradeSchool_sRoster = new WeakMap();
// ================ Tests ===========
